...
A `while` loop is used to run a block of code as long as the condition evaluates to `true`, and the condition is checked before the code block is executed. `while` loops can come in handy in situations like where there is user input and you don't know how many times you'll need to run some code.
