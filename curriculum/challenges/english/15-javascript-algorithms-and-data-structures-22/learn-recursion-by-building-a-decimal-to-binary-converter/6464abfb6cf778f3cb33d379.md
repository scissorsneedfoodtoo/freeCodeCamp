...
For the next phase of the animation, you'll update the paragraphs with the `msg` text, then remove the paragraphs once the animation is complete.

Use the `.forEach()` method to loop through the `animationData` array again. For the `.forEach()` method's callback function, pass in `obj` as a parameter, but leave the body of the callback function empty for now.
