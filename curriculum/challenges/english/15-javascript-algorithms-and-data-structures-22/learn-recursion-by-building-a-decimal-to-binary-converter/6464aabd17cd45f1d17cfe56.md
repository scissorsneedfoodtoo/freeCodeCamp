...
Add the property `msg` set to the string `decimalToBinary(2) returns "1" + 0 (2 % 2) and gives that value to the stack below. Then it pops off the stack.`. Also, add the property `showMsgDelay` set to `10000` and the property `removeElDelay` set to `15000`.
