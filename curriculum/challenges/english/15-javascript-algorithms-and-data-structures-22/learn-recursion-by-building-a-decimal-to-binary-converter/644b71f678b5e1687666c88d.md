...
Inside your `while` loop, divide `input` by `2` and use the assignment operator (`=`) to assign the quotient to `input`. This will update the value of `input` so that the loop can eventually stop running.
